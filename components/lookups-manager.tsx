...
import { addLookup, deleteLookup } from "../app/(app)/actions";
...
export default function LookupsManager({ lookups }: { lookups: Lookup[] }) {
  ...
  const byCategory = (cat: LookupCategory) => lookups.filter((l) => l.category === cat);
          ...
          return (
            <div key={cat} className="bg-white border border-slate-200 rounded-2xl p-5">
              <h3 className="text-sm font-semibold text-slate-700 mb-3 capitalize">
                {cat}s
                <span className="ml-2 text-slate-600 font-normal text-xs">({items.length})</span>
              </h3>
              <div className="space-y-1 max-h-80 overflow-y-auto">
                {items.map((item) => (
                  <div key={item.id} className="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-slate-50 group transition-colors">
